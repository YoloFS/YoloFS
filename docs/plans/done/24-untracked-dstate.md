# 24 — Untracked dstate + tombstone with d_type
...
untracked" (follows base state).  Additionally, tombstones lack a `d_type`
...
|-----------|--------------------------------|---------------------------------|
| Untracked | `val == 0`                     | Default; follows base filesystem|
...
Tombstone layout:
    ...
    [63]    0        (tag)
    [62:61] d_type   2 bits
    [60]    1        (in_base, always true)
    [59:0]  0        (reserved + ino=0 + gen=0)

Key properties:
- Zero-initialized dentries are now "untracked" (correct default semantics).
...
- `agfs_dstate_in_base()` simplifies to `(val >> 60) & 1` for all states
  (untracked gives 0, tombstone gives 1, inode/link read the bit).

## Changes

### kmod/agfs.h
1. Update encoding comment (four states).
2. Add `agfs_dstate_is_untracked()` predicate.
...
### kmod/ioctl.c
1. Restore parsing: `packed == 0` → untracked (skip staging packed);
...
Plan 23 adds `Dirent::Untracked` and `Tombstone { dtype }` on the CLI side.
