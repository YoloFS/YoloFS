...
### Kernel changes (`kmod/inode.c` — `yolo_rename`)
...
3. If equal (`is_roundtrip`), skip `kstrdup`/`yolo_dstate_base_path` and set
   `old_di->dstate = (struct yolo_dstate){0}` (passthrough).
...
Removed `yolo_dentry_relpath` wrapper; call sites now use `dentry_path_raw` directly.
...
- `kmod/inode.c` — roundtrip detection in `yolo_rename`
- `kmod/staging.c` — removed `yolo_dentry_relpath` helper
- `kmod/yolofs.h` — removed `yolo_dentry_relpath` declaration
