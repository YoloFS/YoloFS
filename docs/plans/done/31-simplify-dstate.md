...
| gen (16 bits) | ✅ | move to `yolo_inode_info` (per-inode, not per-dentry) |
...
1. **`yolo_open_staged_ino`** — looks up `.yolofs/inodes/<ino>` by name.
...
property.  The yolofs VFS inode (`d_inode(dentry)`) persists across COW —
only `lower_path` is swapped.  Adding `staging_gen` to
`yolo_inode_info` is natural.
...
enum yolo_dkind {
    YOLO_DKIND_UNSET  = 0,
    YOLO_DKIND_STAGED_INODE = 1,
    YOLO_DKIND_REDIRECT     = 2,
    YOLO_DKIND_TOMBSTONE    = 3,
};

struct yolo_dentry_info {
    ...
    enum yolo_dkind         kind;
    bool                    in_base;
    ...
};
```
...
Replace `struct yolo_dstate { u64 val; }` with `enum yolo_dkind`.
Remove all bit-packing, pointer embedding, dtype encoding.

Query functions simplify to trivial field reads.  Remove
`yolo_dentry_is_unset`, `yolo_dentry_is_base_path`,
`yolo_dentry_is_staged_inode`, `yolo_dentry_is_tombstone`, and
`yolo_dentry_in_base` — callers read `YOLO_D(d)->kind` and
`YOLO_D(d)->in_base` directly.  Lookup sets `in_base = true` for
positive base results, so the field is always valid.

### 2. Move gen to yolo_inode_info

Add `u16 staging_gen` to `yolo_inode_info`.  Reads and writes are
...
- **`yolo_create_staged`** — set `YOLO_I(inode)->staging_gen = sbi->gen`
- **`yolo_do_cow`** — set `YOLO_I(inode)->staging_gen = sbi->gen`
- **`yolo_open_staged`** — check `YOLO_I(inode)->staging_gen >= sbi->gen`
- **`yolo_rename`** — no gen update needed (inode data unchanged)
- **Restore inject** — set after `yolo_iget`

### 3. Open staged files via lower_path

Replace `yolo_open_staged_ino(sbi, ino, flags)` with opening directly
...
Replace `yolo_dentry_base_src(old_dentry)` with:

```c
dentry_path_raw(yolo_lower_dentry(old_dentry), buf, sizeof(buf))
```
...
```c
void yolo_dentry_set_staged(struct dentry *d, bool in_base);
void yolo_dentry_set_redirect(struct dentry *d, bool in_base);
void yolo_dentry_unstage(struct dentry *d);  /* → unset */
```

No ino, gen, d_type, or base_copy parameters.  The caller is
responsible for setting `lower_path` (via `yolo_set_lower_path`)
*before* calling `yolo_dentry_set_staged` — this is already the case
today for create and COW, which resolve the inode store path before
updating dstate.

### 6. Restore inject

`yolo_dentry_add_staged_inode` — no gen parameter (set on inode after
`yolo_iget`).

`yolo_dentry_inject_inode` — resolves lower_path via ino, sets
kind/in_base, sets `staging_gen` on the inode, and adds to dcache.

`yolo_dentry_inject_redirect` — resolves lower_path via
...
before deletion).  `yolo_dentry_add_tombstone` no longer needs a
d_type parameter — tombstones are just negative dentries with
`kind = YOLO_DKIND_TOMBSTONE`.
...
- `struct yolo_dstate { u64 val; }` — replaced by `enum yolo_dkind`
- All bit-packing/unpacking (dtype_pack, dtype_unpack, pointer recovery)
- `yolo_dstate_free` — no embedded pointers to free
- `kstrdup` in rename path — no string allocation
- `yolo_open_staged_ino` — open via lower_path instead
- `yolo_dentry_ino`, `yolo_dentry_gen`,
  `yolo_dentry_base_src` — all removed
- `yolo_dentry_is_unset`, `yolo_dentry_is_staged_inode`,
  `yolo_dentry_is_base_path`, `yolo_dentry_is_tombstone`,
  `yolo_dentry_in_base` — callers read fields directly
- `yolo_dentry_is_current` — replaced by `yolo_dentry_is_current()` inline using inode gen

## Files affected

| File | Change |
|------|--------|
| `yolofs.h` | Replace `struct yolo_dstate` with `enum yolo_dkind`; add `in_base` to dentry_info; add `staging_gen` to inode_info; simplify query inlines |
...
| `ioctl.c` | Update wire decode; set staging_gen after yolo_iget |
