...
2. **Rename** `journal_cmd.rs` → `cmd/audit.rs` (command also renamed: `agfs journal` → `agfs audit`)
...
6. **Update** `cli/main.rs`: import from `agfs::cmd::*`, rename `Command::Journal` → `Command::Audit`
...
9. **Update** docs and example.sh: `agfs journal` → `agfs audit`
