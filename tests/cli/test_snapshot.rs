...
/// Creating a snapshot is visible via `yolofs timeline`.
#[test]
fn snapshot_visible_in_timeline() {
    ...
    assert!(
        ...
        "yolofs timeline should list the 'build' snapshot: {timeline}"
    );
}
...
fn snapshot_list() {
    ...
    let output = s.cli(&["timeline"]).expect("snapshot list");
...
/// Travel to a snapshot reverts the mount to the snapshot state.
#[test]
fn travel_to_snapshot() {
    let s = YoloSession::new().expect("session setup");
    ...
    assert!(
        ...
        "post-snapshot file should not be visible after travel"
    );
}
...
fn multiple_snapshots_interleaved_writes() {
    ...
    // The timeline should list both snapshots.
    let timeline = s.cli(&["timeline"]).expect("timeline");
    assert!(
...
/// `yolofs timeline` shows both snapshot and travel events.
#[test]
fn timeline_shows_travel_events() {
    let s = YoloSession::new().expect("session setup");
    ...
    assert!(
        timeline.contains("build"),
        "timeline should list the 'build' snapshot: {timeline}"
    );
    assert!(
...
/// `yolofs timeline` interleaves snapshots and travels in chronological order.
#[test]
fn timeline_interleaves_snapshots_and_travels() {
    let s = YoloSession::new().expect("session setup");
    ...
    assert!(
        timeline.contains("travel"),
        "timeline should show travel: {timeline}"
    );

    // Travel should appear between chk2 and chk3 in the output
    let travel_pos = timeline.find("travel").unwrap();
    let chk2_pos = timeline.find("chk2").unwrap();
    let chk3_pos = timeline.find("chk3").unwrap();
    assert!(
        travel_pos > chk2_pos && travel_pos < chk3_pos,
        "travel should appear between chk2 and chk3 in timeline: {timeline}"
    );
}
