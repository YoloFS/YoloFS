...
/// Directory stat should raise an ask on the directory itself.
#[test]
fn watch_logs_directory_stat_ask() {
    let s = AgfsSession::new_with_config(Config {
        ask_default: Some(Perm::Deny),
        rules: BTreeMap::from([("/".into(), Perm::Allow)]),
        ..Default::default()
    ...
    let meta = std::fs::metadata(s.mnt_path("subdir"));

    watch.kill().ok();
    let output = watch.wait_with_output().expect("collecting watch output");
    let stderr = String::from_utf8_lossy(&output.stderr).to_string();

    let expected_path = format!("{}/subdir", s.root.display());
    assert!(
        stderr.contains(&expected_path),
        "watch daemon should have received an ask for {expected_path}, got:\n{stderr}"
    );
...
/// directory when resolving a child path.
#[test]
fn watch_logs_directory_traversal_ask() {
    let s = AgfsSession::new_with_config(Config {
        ask_default: Some(Perm::Deny),
        rules: BTreeMap::from([("/".into(), Perm::Allow)]),
        ..Default::default()
    ...
    let meta = std::fs::metadata(s.mnt_path("subdir/deep.txt"));
